...
class head_attention_weighted(nn.Module):
...
class MHA_weighted(nn.Module):
    def __init__(self, d, num_heads, drop=0.0):  
        super().__init__()
        d_head = d // num_heads
        self.heads = nn.ModuleList([head_attention_weighted(d, d_head, drop) for _ in range(num_heads)])
...
class BlockGT(nn.Module):
    def __init__(self, d, num_heads, drop=0.0):  
        ...
        self.MHA = MHA_weighted(d, num_heads, drop)
