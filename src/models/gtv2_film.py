...
class head_attention_film(nn.Module):
...
class MHA_film(nn.Module):
    def __init__(self, d, num_heads, drop=0.0):  
        super().__init__()
        d_head = d // num_heads
        self.heads = nn.ModuleList([head_attention_film(d, d_head, drop) for _ in range(num_heads)])
...
class BlockGT(nn.Module):
    def __init__(self, d, num_heads, drop=0.0):  
        ...
        self.MHA = MHA_film(d, num_heads, drop)
