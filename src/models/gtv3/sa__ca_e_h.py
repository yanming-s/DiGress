...
class head_attention_v3(nn.Module):
    def __init__(self, d, d_head, drop):
        super().__init__()
        self.cross_att_edge_to_node = attention_node_to_node(d, d_head, drop)
        self.cross_att_node_to_edge = attention_node_to_edge(d, d_head, drop)
    def forward(self, x, e, node_mask):
        x_new, _ = self.cross_att_edge_to_node(x, e, node_mask)
        _, e_new = self.cross_att_node_to_edge(x_new, e, node_mask)
        return x_new, e_new
