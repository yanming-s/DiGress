...
from src.models.attention import attention_node_to_node
...
class head_attention_v3(nn.Module):
    def __init__(self, d, d_head, drop):
        ...
        self.cross_att_edge_to_node = attention_edge_to_node(d_head, d_head, drop)
    def forward(self, x, e, node_mask):
        x_hat, _ = self.self_att_node_to_node(x, e, node_mask)
        _, e_new = self.cross_att_node_to_edge(x_hat, e, node_mask)
        x_new, _ = self.cross_att_edge_to_node(x_hat, e_new, node_mask)
        return x_new, e_new
